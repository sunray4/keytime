...
export async function handleHeartbeat(message: Message) {
    ...
    if (user) {
        ...
        if (lastHeartbeat != 0 && timestamp - lastHeartbeat <= maxInterval)  {
            const timeSpent = timestamp - lastHeartbeat;
        ...
        await prisma.user.update({
            where: { id: user!.id },
            data: { lastHeartbeat: timestamp, lastFolder: folder, lastLang: lang, lastEditor: editor }
        });
        return;
    }
}
